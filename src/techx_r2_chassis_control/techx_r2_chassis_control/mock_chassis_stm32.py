...
class MockChassisSTM32(Node):
    ...
    It supports:
      ...
      - STEP_COMMAND:
          result_mode=success: ACK -> RUNNING -> DONE
          result_mode=error:   ACK -> RUNNING -> ERROR
          result_mode=no_ack:  no ACK / no response
          result_mode=timeout: ACK -> RUNNING -> no final DONE/ERROR
      - LIFT_CONTROL:
          result_mode=success: ACK -> RUNNING -> DONE
          result_mode=error:   ACK -> RUNNING -> ERROR
          result_mode=no_ack:  no ACK / no response
    ...
    def __init__(self) -> None:
        ...
        # 新参数：统一机械臂和底盘 mock 的测试风格。
        # 可选：legacy / success / error / no_ack / timeout
        self.declare_parameter("result_mode", "legacy")
        ...
        self.get_logger().info(
            f"Mock STM32 opened serial port {self.port} @ {self.baudrate}"
        )
        self.get_logger().info(
            f"Mock chassis result_mode={self._get_result_mode()}, "
            f"drop_first_ack={bool(self.get_parameter('drop_first_ack').value)}"
        )
    ...
    def _get_result_mode(self) -> str:
        result_mode = str(self.get_parameter("result_mode").value).strip().lower()

        valid_modes = ("success", "error", "no_ack", "timeout")
        if result_mode not in valid_modes:
            self.get_logger().warn(
                f"unknown result_mode={result_mode}, fallback to error"
            )
            return "error"
    ...
    def _reliable_task_busy(self) -> bool:
        return (
            self._current_state in (STATE_ACK, STATE_RUNNING)
            or self._lift_state in (STATE_ACK, STATE_RUNNING)
        )
    ...
    def _send_step_status(
        ...
        self.get_logger().info(
            f"Mock TX STEP_STATUS seq={seq} "
            f"cmd={STEP_CMD_ID_TO_NAME.get(step_cmd, step_cmd)} "
            f"state={STATE_NAME.get(state, state)} "
            f"error=0x{error_code:02X} progress={progress}"
        )

    def _send_lift_status(
        self,
        seq: int,
        state: int,
        error_code: int = ERROR_OK,
    ...
    def _handle_frame(self, frame) -> None:
        ...
        if frame.cmd_type == CMD_CHASSIS_VEL:
            ...
            if bool(self.get_parameter("log_velocity").value):
                self.get_logger().info(
                    f"Mock RX CHASSIS_VEL seq={frame.seq} "
                    f"vx={vx} vy={vy} omega={omega} mode={mode}"
                )
            return
        ...
        if frame.cmd_type == CMD_ESTOP:
            self.get_logger().error(
                f"Mock RX ESTOP seq={frame.seq}; stop chassis/lift/climb immediately"
            )
            with self._state_lock:
        ...
        self.get_logger().warn(
            f"Mock ignore unknown cmd_type=0x{frame.cmd_type:02X}, seq={frame.seq}"
        )
    ...
    def _handle_step_command(self, seq: int, data: bytes) -> None:
        ...
        result_mode = self._get_result_mode()

        if result_mode == "no_ack":
            self.get_logger().warn(
                f"result_mode=no_ack, ignore STEP_COMMAND seq={seq} cmd={cmd_name}"
            )
            return
        ...
        with self._state_lock:
            ...
            if duplicate:
                ...
                self.get_logger().warn(
                    f"Mock duplicate STEP_COMMAND seq={seq}; "
                    f"resend current state={STATE_NAME.get(state, state)}"
                )
            ...
            if self._reliable_task_busy():
                self.get_logger().warn(
                    f"Mock busy, reject new STEP_COMMAND seq={seq}; "
                    "first version returns MOTOR_ERROR"
                )
                self._send_step_status(
                    seq,
                    step_cmd,
                    STATE_ERROR,
                    ERROR_MOTOR_ERROR,
        ...
        self.get_logger().info(
            f"Mock RX STEP_COMMAND seq={seq} cmd={cmd_name} "
            f"delta_h={delta_h} edge_id={edge_id} flags=0x{flags:02X}"
        )

        if bool(self.get_parameter("drop_first_ack").value) and not self._first_ack_dropped:
            self._first_ack_dropped = True
            self.get_logger().warn(
                "Mock drop first reliable-task ACK once, for host resend test"
            )
        else:
            self._send_step_status(seq, step_cmd, STATE_ACK, ERROR_OK, 0)

        threading.Thread(
            target=self._finish_step_task,
            args=(seq, step_cmd),
            daemon=True,
        ).start()
    ...
    def _finish_step_task(self, seq: int, step_cmd: int) -> None:
        ...
        if result_mode == "timeout":
            self.get_logger().warn(
                f"result_mode=timeout, do not send final STEP_STATUS DONE/ERROR seq={seq}"
            )
            return
    ...
    def _handle_lift_control(self, seq: int, data: bytes) -> None:
        ...
        if result_mode == "no_ack":
            self.get_logger().warn(
                f"result_mode=no_ack, ignore LIFT_CONTROL seq={seq} "
                f"target_h={target_h} mask=0x{mask:02X}"
            )
            return
        ...
        with self._state_lock:
            ...
            if duplicate:
                ...
                error = self._lift_error
                self.get_logger().warn(
                    f"Mock duplicate LIFT_CONTROL seq={seq}; "
                    f"resend current state={STATE_NAME.get(state, state)}"
                )
            ...
            if mask == 0 or (mask & ~0x07):
                self.get_logger().warn(
                    f"Mock reject LIFT_CONTROL seq={seq}; invalid mask=0x{mask:02X}"
                )
            ...
            if self._reliable_task_busy():
                self.get_logger().warn(
                    f"Mock busy, reject new LIFT_CONTROL seq={seq}; "
                    "first version returns MOTOR_ERROR"
                )
                self._send_lift_status(seq, STATE_ERROR, ERROR_MOTOR_ERROR)
                return
        ...
        self.get_logger().info(
            f"Mock RX LIFT_CONTROL seq={seq} target_h={target_h} mask=0x{mask:02X}"
        )
        ...
        threading.Thread(
            target=self._finish_lift_task,
            args=(seq, target_h, mask),
            daemon=True,
        ).start()
    ...
    def _finish_lift_task(self, seq: int, target_h: int, mask: int) -> None:
        ...
        if result_mode == "timeout":
            self.get_logger().warn(
                f"result_mode=timeout, do not send final LIFT_STATUS DONE/ERROR seq={seq}"
            )
            return
