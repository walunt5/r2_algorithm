...
def generate_launch_description():
    ...
    # 是否同时启动真实底盘串口节点
    launch_chassis_serial_arg = DeclareLaunchArgument(
        "launch_chassis_serial",
        default_value="true",
        description="Launch techx_r2_chassis_control chassis_serial node.",
    )
    ...
    with open(pcd2depth_config_path, "r", encoding="utf-8") as f:
    ...
    with open(reprojection_config_path, "r", encoding="utf-8") as f:
    ...
    chassis_serial_launch = IncludeLaunchDescription(
        ...
        condition=IfCondition(LaunchConfiguration("launch_chassis_serial")),
    )
    ...
    ld.add_action(launch_chassis_serial_arg)
    ...
    # 新增：和 Odin SLAM 一起启动底盘串口
    ld.add_action(chassis_serial_launch)
