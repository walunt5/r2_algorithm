...
#include "techx_r2_chassis_interfaces/action/lift_control.hpp"
...
class R2LiftControlNode : public BT::StatefulActionNode
{
public:
  using LiftControl = techx_r2_chassis_interfaces::action::LiftControl;
  using GoalHandleLiftControl = rclcpp_action::ClientGoalHandle<LiftControl>;
...
private:
  ...
  rclcpp::Node::SharedPtr node_;
  rclcpp_action::Client<LiftControl>::SharedPtr client_;

  std::string action_name_;
  Stage stage_;

  std::shared_future<GoalHandleLiftControl::SharedPtr> goal_handle_future_;
  GoalHandleLiftControl::SharedPtr goal_handle_;
  std::shared_future<GoalHandleLiftControl::WrappedResult> result_future_;
